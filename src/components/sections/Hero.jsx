  ...
  return (
    <section className="w-full bg-white overflow-hidden h-fit">
      <div className="max-w-7xl mx-auto lg:px-6 lg:py-12 grid grid-cols-1 lg:grid-cols-2 lg:gap-10 items-center">
        ...
        {/* Image + overlay wrapper (mobile: full bleed with overlaid text; desktop: image only, right column) */}
        <div className="relative w-full h-150 sm:h-160 md:h-170 lg:h-115 order-1 lg:order-2">
          <img
            src={hero.image}
          ...
          <div className="absolute inset-0 flex flex-col justify-start p-6 sm:p-8 lg:hidden">
            ...
            <svg width="48" height="4" viewBox="0 0 48 4" fill="none" xmlns="http://www.w3.org/2000/svg" className="my-4">
              <rect width="48" height="4" rx="2" fill="#16a34a" />
            </svg>

            <p className="text-gray-500 mt-5 max-w-[65%] sm:max-w-xs font-medium text-balance">
              {hero.subtext}
            </p>
          </div>
        ...
        <div className="relative z-10 order-2 lg:order-1 hidden lg:block">
          <div className="w-12 h-12 rounded-full bg-green-100 flex items-center justify-center mb-6">
            <HeartHandsIcon className="text-green-700" style={{ width: 22, height: 22 }} />
          </div>
